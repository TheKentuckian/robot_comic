"""Tests for ``GeminiLLMAdapter`` — Phase 4c.2 adapter wiring.
...
without needing a real Gemini client.

The stub matches the surface of ``GeminiTextResponseHandler._call_llm``
(``gemini_text_base.py:125``) which itself returns the same shape as
``BaseLlamaResponseHandler._call_llm``: the tool-call dicts are already in
llama-server form because ``GeminiLLMClient.call_completion``
(``gemini_llm.py``) does the Gemini-native → llama-server conversion before
the handler ever sees them. The adapter's tool-call conversion path is
therefore identical to ``LlamaLLMAdapter``'s.
"""
...
async def test_chat_returns_llm_response_with_text() -> None:
    handler = _StubGeminiHandler(responses=[("hello there", [], {})])
    adapter = GeminiLLMAdapter(handler)  # type: ignore[arg-type]
...
async def test_chat_converts_raw_tool_calls_to_protocol_tool_calls() -> None:
    ...
    Note: ``GeminiLLMClient.call_completion`` emits dicts already in
    llama-server shape — the Gemini-native conversion happens upstream, not
    in the adapter. The adapter's conversion is therefore identical to
    ``LlamaLLMAdapter``'s.
    """
    raw_tool_calls = [
        {
    ...
    handler = _StubGeminiHandler(responses=[("", raw_tool_calls, {})])
    adapter = GeminiLLMAdapter(handler)  # type: ignore[arg-type]
...
async def test_chat_accepts_and_ignores_tools_kwarg() -> None:
    ...
    it — ``GeminiTextResponseHandler._build_llm_messages`` sources tools from
    ``deps`` via ``get_active_tool_specs``. Phase 4 will pass tools through
    cleanly."""
    handler = _StubGeminiHandler(responses=[("ok", [], {})])
    adapter = GeminiLLMAdapter(handler)  # type: ignore[arg-type]
...
async def test_shutdown_with_no_open_http_is_safe() -> None:
    handler = _StubGeminiHandler()  # _http already None
    adapter = GeminiLLMAdapter(handler)  # type: ignore[arg-type]
...
def test_adapter_satisfies_llm_backend_protocol() -> None:
    """``GeminiLLMAdapter`` passes isinstance(LLMBackend)."""
    from robot_comic.backends import LLMBackend

    adapter = GeminiLLMAdapter(_StubGeminiHandler())  # type: ignore[arg-type]
