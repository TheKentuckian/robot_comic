...
async def test_chat_records_llm_duration_on_success(
    ...
    The legacy ``BaseLlamaResponseHandler._run_response_loop`` brackets
    ``_call_llm`` with ``time.perf_counter()`` and calls
    ``telemetry.record_llm_duration``. The composable path goes through
    ``LlamaLLMAdapter.chat()`` instead, which calls ``_call_llm`` directly
    and therefore bypasses the legacy site. The adapter must wrap its own
    timing so the gen_ai.client.operation.duration histogram fires
    regardless of path.
    """
    from robot_comic.adapters import llama_llm_adapter as mod
    ...
    assert attrs == {"gen_ai.system": "llama_cpp", "gen_ai.operation.name": "chat"}
