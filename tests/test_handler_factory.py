...
class TestHandlerFactoryComposableCombinations:
    ...
    def test_moonshine_llama_chatterbox_uses_standalone_moonshine_adapter(self, mock_deps: MagicMock) -> None:
        """Phase 5e.3: the migrated triple constructs a standalone STT adapter.

        Before 5e.3 the (moonshine, llama, chatterbox) builder constructed
        ``MoonshineSTTAdapter(host)`` (host-coupled, ``_handler`` set).
        Post-5e.3 it constructs ``MoonshineSTTAdapter()`` (standalone,
        ``_handler is None``) just like the 5e.2 elevenlabs sibling.
        """
        with patch("robot_comic.handler_factory.config") as mock_cfg:
            mock_cfg.LLM_BACKEND = "llama"
        ...
        assert stt_adapter._handler is None, (
            "Phase 5e.3 migrated triple must construct standalone "
            "MoonshineSTTAdapter (handler=None); got host-coupled."
        )

    def test_moonshine_llama_chatterbox_wires_should_drop_frame_callback(self, mock_deps: MagicMock) -> None:
    ...
    def test_moonshine_llama_chatterbox_passes_deps_to_pipeline(self, mock_deps: MagicMock) -> None:
    ...
    def test_moonshine_llama_elevenlabs_uses_standalone_moonshine_adapter(self, mock_deps: MagicMock) -> None:
        ...
        ``MoonshineSTTAdapter(host)`` (host-coupled, ``_handler`` set).
        Post-5e.2 the (moonshine, llama, elevenlabs) builder constructs
        ...
        stt_adapter = result.pipeline.stt
        assert stt_adapter._handler is None, (
            "Phase 5e.2 migrated triple must construct standalone "
            "MoonshineSTTAdapter (handler=None); got host-coupled."
        )
    ...
    def test_moonshine_gemini_tts_uses_standalone_moonshine_adapter(self, mock_deps: MagicMock) -> None:
        """Phase 5e.6: the final migrated triple constructs a standalone STT adapter.

        Before 5e.6 the (moonshine, gemini-bundled, gemini_tts) builder
        constructed ``MoonshineSTTAdapter(host)`` (host-coupled,
        ``_handler`` set). Post-5e.6 it constructs
        ``MoonshineSTTAdapter()`` (standalone, ``_handler is None``)
        just like the 5e.2 / 5e.3 / 5e.4 / 5e.5 siblings — completing
        the mixin-retirement arc for all five composable triples.
        """
        with patch("robot_comic.handler_factory.config") as mock_cfg:
            mock_cfg.LLM_BACKEND = "llama"
            mock_cfg.WELCOME_GATE_ENABLED = False
        ...
        stt_adapter = result.pipeline.stt
        assert stt_adapter._handler is None, (
            "Phase 5e.6 migrated triple must construct standalone "
            "MoonshineSTTAdapter (handler=None); got host-coupled."
        )

    def test_moonshine_gemini_tts_wires_should_drop_frame_callback(self, mock_deps: MagicMock) -> None:
        """The factory wires the echo-guard ``should_drop_frame`` closure.

        Note: :class:`GeminiTTSResponseHandler` does not write
        ``_speaking_until`` (unlike the ElevenLabs / Chatterbox /
        Llama bases that the other four triples inherit from), so the
        closure's ``getattr(host, "_speaking_until", 0.0)`` always
        returns ``0.0`` → the echo-guard is a documented no-op for
        this triple. We wire it uniformly across all five triples to
        keep the factory builders mechanically identical.
        """
        with patch("robot_comic.handler_factory.config") as mock_cfg:
            mock_cfg.LLM_BACKEND = "llama"
            mock_cfg.WELCOME_GATE_ENABLED = False
    ...
    def test_moonshine_gemini_tts_passes_deps_to_pipeline(self, mock_deps: MagicMock) -> None:
        """The pipeline receives ``deps`` so the host-concern callbacks fire."""
        with patch("robot_comic.handler_factory.config") as mock_cfg:
            mock_cfg.LLM_BACKEND = "llama"
            mock_cfg.WELCOME_GATE_ENABLED = False
    ...
    def test_moonshine_gemini_elevenlabs_uses_standalone_moonshine_adapter(self, mock_deps: MagicMock) -> None:
        """Phase 5e.5: the migrated triple constructs a standalone STT adapter.

        Before 5e.5 the (moonshine, gemini, elevenlabs) builder
        constructed ``MoonshineSTTAdapter(host)`` (host-coupled,
        ``_handler`` set). Post-5e.5 it constructs
        ``MoonshineSTTAdapter()`` (standalone, ``_handler is None``)
        just like the 5e.2 / 5e.3 / 5e.4 siblings.
        """
        with patch("robot_comic.handler_factory.config") as mock_cfg:
            mock_cfg.LLM_BACKEND = "gemini"
            mock_cfg.WELCOME_GATE_ENABLED = False
        ...
        stt_adapter = result.pipeline.stt
        assert stt_adapter._handler is None, (
            "Phase 5e.5 migrated triple must construct standalone "
            "MoonshineSTTAdapter (handler=None); got host-coupled."
        )

    def test_moonshine_gemini_elevenlabs_wires_should_drop_frame_callback(self, mock_deps: MagicMock) -> None:
        """The factory wires the echo-guard ``should_drop_frame`` closure."""
        with patch("robot_comic.handler_factory.config") as mock_cfg:
            mock_cfg.LLM_BACKEND = "gemini"
            mock_cfg.WELCOME_GATE_ENABLED = False
    ...
    def test_moonshine_gemini_elevenlabs_passes_deps_to_pipeline(self, mock_deps: MagicMock) -> None:
        """The pipeline receives ``deps`` so the host-concern callbacks fire."""
        with patch("robot_comic.handler_factory.config") as mock_cfg:
            mock_cfg.LLM_BACKEND = "gemini"
            mock_cfg.WELCOME_GATE_ENABLED = False
